/**
 * Servicio de Supabase para obtener m├⌐tricas de delivery
...
if (!supabaseUrl || !supabaseAnonKey) {
  ...
  console.warn('💡 Si las variables están en .env, reinicia el servidor de desarrollo: npm run dev');
}
...
if (import.meta.env.DEV) {
  if (supabase) {
    ...
    console.error('   Verifica que VITE_SUPABASE_URL y VITE_SUPABASE_ANON_KEY estén en .env');
    console.error('   Reinicia el servidor de desarrollo: npm run dev');
  }
}

/**
 * Obtiene m├⌐tricas de sprints desde Supabase
 * @param {string} projectKey - Clave del proyecto (default: 'OBD')
 * @param {Object} options - Opciones adicionales
 * @returns {Promise<Array>} Array de m├⌐tricas de sprints
 */
export const getSprintMetrics = async (projectKey = 'OBD', options = {}) => {
  if (!supabase) {
    throw new Error('Supabase no est├í configurado. Verifica las variables de entorno.');
  }
  ...
  try {
    ...
    // Ordenar por fecha de fin (m├ís recientes primero)
    ...
    if (error) {
      console.error('[SUPABASE] Error obteniendo m├⌐tricas de sprints:', error);
      throw error;
    }
...
/**
 * Obtiene m├⌐tricas de desarrolladores por sprint
 * @param {string} projectKey - Clave del proyecto (default: 'OBD')
 * @param {Object} options - Opciones adicionales
 * @returns {Promise<Array>} Array de m├⌐tricas de desarrolladores
 */
export const getDeveloperMetrics = async (projectKey = 'OBD', options = {}) => {
  ...
  try {
    ...
    // Filtrar por sprint espec├¡fico si se especifica
    if (options.sprintName) {
      query = query.eq('sprint_name', options.sprintName);
    }

    // Filtrar por desarrollador espec├¡fico si se especifica
    ...
    if (error) {
      console.error('[SUPABASE] Error obteniendo m├⌐tricas de desarrolladores:', error);
      throw error;
    }
...
/**
 * Obtiene m├⌐tricas globales del proyecto
 * @param {string} projectKey - Clave del proyecto (default: 'OBD')
 * @returns {Promise<Object>} Objeto con m├⌐tricas globales
 */
export const getGlobalMetrics = async (projectKey = 'OBD') => {
  ...
  try {
    ...
    if (error) {
      console.error('[SUPABASE] Error obteniendo m├⌐tricas globales:', error);
      throw error;
    }
...
export const getActiveSprint = async (projectKey = 'OBD') => {
  ...
  try {
    ...
    if (error) {
      // Si no hay sprint activo, no es un error cr├¡tico
 ...
 * @returns {Promise<Array>} Array de proyectos con m├⌐tricas
 */
export const getDeliveryRoadmapData = async () => {
  ...
  try {
    ...
    // Obtener m├⌐tricas de sprints m├ís recientes
    ...
    if (metricsError) {
      console.warn('[SUPABASE] Error obteniendo m├⌐tricas:', metricsError);
    }
    ...
    for (const initiative of initiatives || []) {
      ...
      // Obtener fechas del sprint m├ís reciente de este squad
...
/**
 * Obtiene datos de asignaci├│n de desarrolladores desde Supabase
...
export const getDeveloperAllocationData = async () => {
  ...
  try {
    // Obtener sprints activos o m├ís recientes por squad
    ...
    // Crear mapa de sprint m├ís reciente por squad
    ...
    // Obtener relaciones issue-sprint para saber en qu├⌐ sprint est├í cada issue
    ...
    // Funci├│n para verificar si un issue est├í activo en el sprint actual
    const isIssueActiveInSprint = (issue, sprint) => {
      ...
      // Si el sprint ya termin├│ hace m├ís de un sprint, no contar
      if (sprintEnd < now && sprint.state === 'closed') {
        // Solo contar si termin├│ recientemente (├║ltimo sprint cerrado)
        const daysSinceEnd = (now - sprintEnd) / (1000 * 60 * 60 * 24);
        if (daysSinceEnd > 14) return false; // M├ís de 2 semanas = no contar
      }

      // PRIORIDAD 1: Verificar si el issue est├í expl├¡citamente en este sprint (issue_sprints)
      // Esta es la fuente m├ís confiable
      const issueSprintIds = issueSprintMap.get(issue.id) || [];
      if (issueSprintIds.includes(sprint.id)) {
        return true; // Est├í en el sprint, contar
      }

      // PRIORIDAD 2: Si NO est├í en issue_sprints, verificar fechas SOLO para tickets muy recientes
      ...
      // Solo contamos si est├í expl├¡citamente en el sprint o fue creado durante el sprint
      return false;
    };
    ...
    // Crear mapas para b├║squeda r├ípida
    ...
    // Capacidad del sprint basada en la tabla de conversi├│n:
    // 1 SP = 4 horas
    // 2 SP = 1 d├¡a (8 horas)
    // 3 SP = 2-3 d├¡as (16-24 horas)
    // 5 SP = 3-4 d├¡as (24-32 horas)
    // Sprint = 2 semanas = 8.5 d├¡as de trabajo = 68 horas
    ...
    const allocations = Array.from(allocationMap.values()).map(allocation => {
      ...
      // No limitamos a 100% porque un desarrollador puede estar asignado a m├║ltiples iniciativas
    ...
    if (allocations.length === 0) {
      ...
      // Retornar array vac├¡o en lugar de lanzar error, ya que puede ser v├ílido
    }
...
export const testConnection = async () => {
  ...
  try {
    ...
    if (error) {
      console.error('[SUPABASE] Error de conexi├│n:', error);
      return false;
    }
