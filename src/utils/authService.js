 ...
 * Maneja autenticaci├│n y sesiones de usuarios
 */
...
const getSupabase = () => {
    ...
    // No podemos hacer import síncrono aquí, se manejará dinámicamente en las funciones
    return null;
};

// Generar token simple (en producci├│n usar JWT o similar)
const generateToken = () => {
...
export const login = async (email, password) => {
  // Obtener supabase desde window o importar din├ímicamente
  ...
  if (!supabaseClient) {
    return { success: false, error: 'Supabase no est├í configurado' };
  }

  try {
    // Hash simple del password (en producci├│n usar bcrypt o similar)
    // Por ahora, para desarrollo, usamos un hash simple
    const passwordHash = btoa(password); // Base64 encoding (temporal, no seguro para producci├│n)
    ...
    if (error) {
      console.error('[AUTH] Error en autenticaci├│n:', error);
    ...
    if (sessionError) {
      console.error('[AUTH] Error creando sesi├│n:', sessionError);
...
export const getSession = () => {
  try {
    ...
    console.error('[AUTH] Error leyendo sesi├│n:', error);
    return null;
  }
};
...
export const validateSession = async () => {
  ...
  try {
    ...
    console.error('[AUTH] Error validando sesi├│n:', error);
    return { valid: false };
  }
};
...
export const requestPasswordReset = async (email) => {
  ...
  try {
    ...
    // Por seguridad, siempre retornamos ├⌐xito (no revelamos si el email existe)
    return {
      success: true,
      token: data
    };
...
export const resetPassword = async (token, newPassword) => {
  ...
  try {
    ...
    if (error) {
      console.error('[AUTH] Error reseteando contrase├▒a:', error);
      return { success: false, error: error.message };
    }
